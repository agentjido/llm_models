defmodule LLMDB.Store do
      ...
      %{models: models_by_provider, providers_by_id: providers_by_id} ->
        ...
        aliased_models =
          ...
          |> Enum.filter(fn {_id, provider} ->
            Map.get(provider, :alias_of) == provider_id || Map.get(provider, "alias_of") == Atom.to_string(provider_id)
          end)
  ...
  def model(provider_id, model_id) when is_atom(provider_id) and is_binary(model_id) do
    case snapshot() do
      %{models_by_key: models_by_key, aliases_by_key: aliases_by_key, providers_by_id: providers_by_id} ->
